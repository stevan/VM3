...
subtest '... simple duration object' => sub {
    ...
    subtest '... test equality' => sub {
        my $d2 = VM::Timers::Duration->new->in_milliseconds( $d->as_milliseconds );
    ...
    subtest '... test inequality' => sub {
        my $d2 = VM::Timers::Duration->new->in_milliseconds( $d->as_milliseconds + 10 );
        isa_ok($d2, 'VM::Timers::Duration');
...
subtest '... simple duration object in milliseconds' => sub {
    my $d = VM::Timers::Duration->new->in_milliseconds( 2056 );
    isa_ok($d,'VM::Timers::Duration');

    is($d->as_milliseconds, 2056, '... got the expected absolute number of milliseconds');

    subtest '... test adding' => sub {
        my $d2 = VM::Timers::Duration->new->in_milliseconds( 100 );
        isa_ok($d2, 'VM::Timers::Duration');

        is($d2->as_milliseconds, 100, '... the new duration is as expected');
        is($d->as_milliseconds, 2056, '... the old duration is as expected');

        my $d3 = $d->add_to( $d2 );
        isa_ok($d3, 'VM::Timers::Duration');

        is($d3->as_milliseconds, 2156, '... the resulting duration is as expected');
    };

    subtest '... test subtraction' => sub {
        my $d2 = VM::Timers::Duration->new->in_milliseconds( 1730 );
        isa_ok($d2, 'VM::Timers::Duration');

        is($d2->as_milliseconds, 1730, '... the new duration is as expected');
        is($d->as_milliseconds, 2056, '... the old duration is as expected');

        my $d3 = $d->subtract_from( $d2 );
        isa_ok($d3, 'VM::Timers::Duration');

        is($d3->as_milliseconds, 326, '... the resulting duration is as expected');
    };

    subtest '... test subtraction' => sub {
        $d->increase_by( 231 );
        is($d->as_milliseconds, 2287, '... the old duration is as expected');
    };
};
...
subtest '... sorting duration objects (seconds)' => sub {
    ...
    my @durations = map {
        VM::Timers::Duration->new->in_seconds( $_ )
    } @seconds;

    my @sorted = sort { $a->compare_to($b) } @durations;

    eq_or_diff(
        [ map $_->as_seconds, @sorted ],
        [ sort { $a <=> $b } @seconds ],
...
subtest '... sorting duration objects (milliseconds)' => sub {
    ...
    my @durations = map {
        VM::Timers::Duration->new->in_milliseconds( $_ )
    } @seconds;

    my @sorted = sort { $a->compare_to($b) } @durations;

    eq_or_diff(
        [ map $_->as_milliseconds, @sorted ],
        [ sort { $a <=> $b } @seconds ],
        '... sorted the list correctly'
    );
};
